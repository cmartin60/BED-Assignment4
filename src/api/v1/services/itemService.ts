...
/**
 * @description Get items by a specific field value.
 ...
 * @returns {Promise<Item[]>} Array of items matching the criteria.
 * @throws {Error} If no items with the given field value are found or if the query fails.
 *
 * @example
 * // Get active items
 * const activeItems = await getItemsByField('status', 'active');
 *
 * @example
 * // Get top 5 items by category
 * const topCategoryItems = await getItemsByField('category', 'electronics', 5);
 */
